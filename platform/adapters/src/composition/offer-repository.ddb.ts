 ...
 * OfferRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing Offer repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { OfferRepository } from "@loadline/services/composition";
...
/**
 * OfferRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: PRICE_OFFER
 * Entity Token: PRICE_OFFER
 * SK Prefix: PRICE_OFFER
 */
export class OfferRepositoryDdb implements OfferRepository {
  ...
  private readonly ENTITY_TYPE = "PRICE_OFFER";
  private readonly ENTITY_TOKEN = "PRICE_OFFER";
  private readonly SK_PREFIX = "PRICE_OFFER";
  ...
  private readonly ENTITY_TYPE_PK = "PRICE_OFFERS";
  ...
  async issuePriceOffer(input: Parameters<OfferRepository['issuePriceOffer']>[0]): Promise<Awaited<ReturnType<OfferRepository['issuePriceOffer']>>> {
    ...
    if (!(inputExt.id ?? inputExt.compositionId)) {
      throw new Error("Missing required parameter: id");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.id ?? inputExt.compositionId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.id ?? inputExt.compositionId)}#`;
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.id ?? inputExt.compositionId)} not found`);
    }
    return {
