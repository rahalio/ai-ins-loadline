 ...
 * ReviewRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing Review repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { ReviewRepository } from "@loadline/services/reinsurance";
...
/**
 * ReviewRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: REVIEW_DECISION
 * Entity Token: REVIEW_DECISION
 * SK Prefix: REVIEW_DECISION
 */
export class ReviewRepositoryDdb implements ReviewRepository {
  ...
  private readonly ENTITY_TYPE = "REVIEW_DECISION";
  private readonly ENTITY_TOKEN = "REVIEW_DECISION";
  private readonly SK_PREFIX = "REVIEW_DECISION";
  ...
  private readonly ENTITY_TYPE_PK = "REVIEW_DECISIONS";
  ...
  async requestHumanReview(input: Parameters<ReviewRepository['requestHumanReview']>[0]): Promise<Awaited<ReturnType<ReviewRepository['requestHumanReview']>>> {
    ...
    if (!(inputExt.id ?? inputExt.decisionId)) {
      throw new Error("Missing required parameter: id");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.id ?? inputExt.decisionId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.id ?? inputExt.decisionId)}#`;
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.id ?? inputExt.decisionId)} not found`);
    }
    return {
